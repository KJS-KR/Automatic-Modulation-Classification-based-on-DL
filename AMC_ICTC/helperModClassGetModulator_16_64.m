function modulator = helperModClassGetModulator_16_64(modType, sps, fs, phase_16, phase_64)
...
switch modType
  ...
  case "16QAM"
    modulator = @(x)qam16Modulator(x,sps, phase_16);
  case "64QAM"
    modulator = @(x)qam64Modulator(x,sps, phase_64);
  case "GFSK"
...
function y = qam16Modulator(x,sps, phase_16)
...
function y = qam64Modulator(x,sps, phase_64)
...
% Modulate
syms = qammod(x,64,'UnitAveragePower',true).* exp(1i*pi/phase_64);
% Pulse shape
y = filter(filterCoeffs, 1, upsample(syms,sps));
end
