function modulator = helperModClassGetModulator(modType, sps, fs, phase_q, phase_8)
...
switch modType
  ...
  case "QPSK"
    modulator = @(x)qpskModulator(x,sps,phase_q);
  case "8PSK"
    modulator = @(x)psk8Modulator(x,sps,phase_8);
  case "16QAM"
...
function y = qpskModulator(x,sps,phase_q)
...
% Modulate
syms = pskmod(x,4,pi/phase_q);
% Pulse shape
y = filter(filterCoeffs, 1, upsample(syms,sps));
end

function y = psk8Modulator(x,sps,phase_8)
