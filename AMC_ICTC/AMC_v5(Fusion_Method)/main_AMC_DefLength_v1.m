...
for snrIdx = 1:1:numSnr
    for spfIdx = 1:1:3
        totalSignal = spf(spfIdx) * N_Frame; 
        %% BPSK 신호 생성
        modNumBpsk = 2;
        ...
        sigBpsk = reshape(awgnSigBpsk, spf(spfIdx), N_Frame); 
        ...
        sigQpsk = reshape(awgnSigQpsk, spf(spfIdx), N_Frame);
        ...
        sig8psk = reshape(awgnSig8psk, spf(spfIdx), N_Frame);
        ...
        sig16qam = reshape(awgnSig16qam, spf(spfIdx), N_Frame);
        ...
        sig32qam = reshape(awgnSig32qam, spf(spfIdx), N_Frame);
        ...
        sig64qam = reshape(awgnSig64qam, spf(spfIdx), N_Frame);
        ...
        sig4pam = reshape(awgnSig4pam, spf(spfIdx), N_Frame);
        ...
        sig8pam = reshape(awgnSig8pam, spf(spfIdx), N_Frame);

        numSegment = spf(spfIdx)/segmentLength;
    
        for segIdx = 1:numSegment
    
            SegmentedSigBpsk = sigBpsk(1+segmentLength*(segIdx-1):segmentLength*segIdx,:);
        
            dataBpsk(:,1,:) = real(SegmentedSigBpsk);
            dataBpsk(:,2,:) = imag(SegmentedSigBpsk);
    
            SegmentedSigQpsk = sigQpsk(1+segmentLength*(segIdx-1):segmentLength*segIdx,:);
        
            dataQpsk(:,1,:) = real(SegmentedSigQpsk);
            dataQpsk(:,2,:) = imag(SegmentedSigQpsk);
    
            SegmentedSig8psk = sig8psk(1+segmentLength*(segIdx-1):segmentLength*segIdx,:);
        
            data8psk(:,1,:) = real(SegmentedSig8psk);
            data8psk(:,2,:) = imag(SegmentedSig8psk);
    
            SegmentedSig16qam = sig16qam(1+segmentLength*(segIdx-1):segmentLength*segIdx,:);
        
            data16qam(:,1,:) = real(SegmentedSig16qam);
            data16qam(:,2,:) = imag(SegmentedSig16qam);
    
            SegmentedSig32qam = sig32qam(1+segmentLength*(segIdx-1):segmentLength*segIdx,:);
        
            data32qam(:,1,:) = real(SegmentedSig32qam);
            data32qam(:,2,:) = imag(SegmentedSig32qam);
    
            SegmentedSig64qam = sig64qam(1+segmentLength*(segIdx-1):segmentLength*segIdx,:);
        
            data64qam(:,1,:) = real(SegmentedSig64qam);
            data64qam(:,2,:) = imag(SegmentedSig64qam);
    
            SegmentedSig4pam = sig4pam(1+segmentLength*(segIdx-1):segmentLength*segIdx,:);
        
            data4pam(:,1,:) = real(SegmentedSig4pam);
            data4pam(:,2,:) = imag(SegmentedSig4pam);
    
            SegmentedSig8pam = sig8pam(1+segmentLength*(segIdx-1):segmentLength*segIdx,:);
            ...
            % Total Training Set
            testData(:,:,1:N_Frame*testDataRatio) = dataBpsk(:,:,1:N_Frame*testDataRatio);
            testData(:,:,N_Frame*testDataRatio+1:N_Frame*testDataRatio*2) = dataQpsk(:,:,1:N_Frame*testDataRatio);
            testData(:,:,N_Frame*testDataRatio*2+1:N_Frame*testDataRatio*3) = data8psk(:,:,1:N_Frame*testDataRatio);
            testData(:,:,N_Frame*testDataRatio*3+1:N_Frame*testDataRatio*4) = data16qam(:,:,1:N_Frame*testDataRatio);
            testData(:,:,N_Frame*testDataRatio*4+1:N_Frame*testDataRatio*5) = data32qam(:,:,1:N_Frame*testDataRatio);
            testData(:,:,N_Frame*testDataRatio*5+1:N_Frame*testDataRatio*6) = data64qam(:,:,1:N_Frame*testDataRatio);
            testData(:,:,N_Frame*testDataRatio*6+1:N_Frame*testDataRatio*7) = data4pam(:,:,1:N_Frame*testDataRatio);
            testData(:,:,N_Frame*testDataRatio*7+1:N_Frame*testDataRatio*8) = data8pam(:,:,1:N_Frame*testDataRatio);
