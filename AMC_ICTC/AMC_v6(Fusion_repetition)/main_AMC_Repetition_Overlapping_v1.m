...
overlappingRatio    = [0 0.25 0.5 0.75];
numSegment          = [4 5 7 13];
...
for snrIdx = 1:1:numSnr
    ...
    for rep = 1:numRepetition-2
        sigBpsk = vertcat(sigBpsk, sigBpsk);
    end
    ...
    for rep = 1:numRepetition-2
        sigQpsk = vertcat(sigQpsk, sigQpsk);
    end
    ...
    for rep = 1:numRepetition-2
        sig8psk = vertcat(sig8psk, sig8psk);
    end
    ...
    for rep = 1:numRepetition-2
        sig16qam = vertcat(sig16qam, sig16qam);
    end
    ...
    for rep = 1:numRepetition-2
        sig32qam = vertcat(sig32qam, sig32qam);
    end
    ...
    for rep = 1:numRepetition-2
        sig64qam = vertcat(sig64qam, sig64qam);
    end
    ...
    for rep = 1:numRepetition-2
        sig4pam = vertcat(sig4pam, sig4pam);
    end
    ...
    for rep = 1:numRepetition-2
        sig8pam = vertcat(sig8pam, sig8pam);
    end
    ...
    for overlapIndex = 1:4
        ...
        for segIdx = 1:Segment
            ...
            SegmentedSigBpsk = sigBpsk(1+spf*(segIdx-1)*overlappingRatio:spf+spf*(segIdx-1)*overlappingRatio(overlapIndex),:);
        
            dataBpsk(:,1,:) = real(SegmentedSigBpsk);
            dataBpsk(:,2,:) = imag(SegmentedSigBpsk);
    
            SegmentedSigQpsk = sigQpsk(1+spf*(segIdx-1)*overlappingRatio:spf+spf*(segIdx-1)*overlappingRatio(overlapIndex),:);
        
            dataQpsk(:,1,:) = real(SegmentedSigQpsk);
            dataQpsk(:,2,:) = imag(SegmentedSigQpsk);
    
            SegmentedSig8psk = sig8psk(1+spf*(segIdx-1)*overlappingRatio:spf+spf*(segIdx-1)*overlappingRatio(overlapIndex),:);
        
            data8psk(:,1,:) = real(SegmentedSig8psk);
            data8psk(:,2,:) = imag(SegmentedSig8psk);
    
            SegmentedSig16qam = sig16qam(1+spf*(segIdx-1)*overlappingRatio:spf+spf*(segIdx-1)*overlappingRatio(overlapIndex),:);
        
            data16qam(:,1,:) = real(SegmentedSig16qam);
            data16qam(:,2,:) = imag(SegmentedSig16qam);
    
            SegmentedSig32qam = sig32qam(1+spf*(segIdx-1)*overlappingRatio:spf+spf*(segIdx-1)*overlappingRatio(overlapIndex),:);
        
            data32qam(:,1,:) = real(SegmentedSig32qam);
            data32qam(:,2,:) = imag(SegmentedSig32qam);
    
            SegmentedSig64qam = sig64qam(1+spf*(segIdx-1)*overlappingRatio:spf+spf*(segIdx-1)*overlappingRatio(overlapIndex),:);
        
            data64qam(:,1,:) = real(SegmentedSig64qam);
            data64qam(:,2,:) = imag(SegmentedSig64qam);
    
            SegmentedSig4pam = sig4pam(1+spf*(segIdx-1)*overlappingRatio:spf+spf*(segIdx-1)*overlappingRatio(overlapIndex),:);
        
            data4pam(:,1,:) = real(SegmentedSig4pam);
            data4pam(:,2,:) = imag(SegmentedSig4pam);
    
            SegmentedSig8pam = sig8pam(1+spf*(segIdx-1)*overlappingRatio:spf+spf*(segIdx-1)*overlappingRatio(overlapIndex),:);
...
%% 학습된 신경망을 통한 결과
        ...
        averageFcOutput             = (totalFcOutput/numRepetition).';
