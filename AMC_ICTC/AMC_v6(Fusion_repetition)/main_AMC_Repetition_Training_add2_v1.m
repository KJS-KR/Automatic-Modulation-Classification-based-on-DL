...
%         add1Sig8pam(1:spf*2,frameIdx) = addSig8pam;
        ...
        % (1rep+2rep+3rep+4rep)
        add2Sig4pam(1:spf,frameIdx) = repSig8pam(1:spf,:) + repSig8pam(1+spf:spf*2) + repSig8pam(1+spf*2:spf*3) + repSig8pam(1+spf*3:spf*4);

    end

%% 데이터셋 생성 및 분할

    dataBpsk(:,1,:) = real(addSigBpsk);
    dataBpsk(:,2,:) = imag(addSigBpsk);

    dataQpsk(:,1,:) = real(addSigQpsk);
    dataQpsk(:,2,:) = imag(addSigQpsk);

    data8psk(:,1,:) = real(addSig8psk);
    data8psk(:,2,:) = imag(addSig8psk);

    data16qam(:,1,:) = real(addSig16qam);
    data16qam(:,2,:) = imag(addSig16qam);

    data32qam(:,1,:) = real(addSig32qam);
    data32qam(:,2,:) = imag(addSig32qam);

    data64qam(:,1,:) = real(addSig64qam);
    data64qam(:,2,:) = imag(addSig64qam);

    data4pam(:,1,:) = real(addSig4pam);
    data4pam(:,2,:) = imag(addSig4pam);

    data8pam(:,1,:) = real(addSig8pam);
    data8pam(:,2,:) = imag(addSig8pam);
