...
for snrIdx = 1:numSnr
    ...
    for frameIdx = 1:N_Frame
        ...
        % (1rep+2rep+3rep)
        add3SigBpsk(1:spf,frameIdx) = repSigBpsk(1:spf,:) + repSigBpsk(1+spf:spf*2) + repSigBpsk(1+spf*2:spf*3);
        add3SigBpsk(1+spf:spf*2,frameIdx) = repSigBpsk(1+spf:spf*2) + repSigBpsk(1+spf*2:spf*3) + repSigBpsk(1+spf*3:spf*4);
        ...
        % (1rep+2rep+3rep)
        add3SigQpsk(1:spf,frameIdx) = repSigQpsk(1:spf,:) + repSigQpsk(1+spf:spf*2) + repSigQpsk(1+spf*2:spf*3);
        add3SigQpsk(1+spf:spf*2,frameIdx) = repSigQpsk(1+spf:spf*2) + repSigQpsk(1+spf*2:spf*3) + repSigQpsk(1+spf*3:spf*4);
        ...
        % (1rep+2rep+3rep)
        add3Sig8psk(1:spf,frameIdx) = repSig8psk(1:spf,:) + repSig8psk(1+spf:spf*2) + repSig8psk(1+spf*2:spf*3);
        add3Sig8psk(1+spf:spf*2,frameIdx) = repSig8psk(1+spf:spf*2) + repSig8psk(1+spf*2:spf*3) + repSig8psk(1+spf*3:spf*4);
        ...
        % (1rep+2rep+3rep)
        add3Sig16qam(1:spf,frameIdx) = repSig16qam(1:spf,:) + repSig16qam(1+spf:spf*2) + repSig16qam(1+spf*2:spf*3);
        add3Sig16qam(1+spf:spf*2,frameIdx) = repSig16qam(1+spf:spf*2) + repSig16qam(1+spf*2:spf*3) + repSig16qam(1+spf*3:spf*4);
        ...
        % (1rep+2rep+3rep)
        add3Sig32qam(1:spf,frameIdx) = repSig32qam(1:spf,:) + repSig32qam(1+spf:spf*2) + repSig32qam(1+spf*2:spf*3);
        add3Sig32qam(1+spf:spf*2,frameIdx) = repSig32qam(1+spf:spf*2) + repSig32qam(1+spf*2:spf*3) + repSig32qam(1+spf*3:spf*4);
        ...
        % (1rep+2rep+3rep)
        add3Sig64qam(1:spf,frameIdx) = repSig64qam(1:spf,:) + repSig64qam(1+spf:spf*2) + repSig64qam(1+spf*2:spf*3);
        add3Sig64qam(1+spf:spf*2,frameIdx) = repSig64qam(1+spf:spf*2) + repSig64qam(1+spf*2:spf*3) + repSig64qam(1+spf*3:spf*4);
        ...
        % (1rep+2rep+3rep)
        add3Sig4pam(1:spf,frameIdx) = repSig4pam(1:spf,:) + repSig4pam(1+spf:spf*2) + repSig4pam(1+spf*2:spf*3);
        add3Sig4pam(1+spf:spf*2,frameIdx) = repSig4pam(1+spf:spf*2) + repSig4pam(1+spf*2:spf*3) + repSig4pam(1+spf*3:spf*4);
        ...
        % (1rep+2rep+3rep)
        add3Sig8pam(1:spf,frameIdx) = repSig8pam(1:spf,:) + repSig8pam(1+spf:spf*2) + repSig8pam(1+spf*2:spf*3);
        add3Sig8pam(1+spf:spf*2,frameIdx) = repSig8pam(1+spf:spf*2) + repSig8pam(1+spf*2:spf*3) + repSig8pam(1+spf*3:spf*4);

    end

    for segIdx = 1:2
        SegmentedSigBpsk = add3SigBpsk(1+spf*(segIdx-1):spf*segIdx,:);
    
        dataBpsk(:,1,:) = real(SegmentedSigBpsk);
        dataBpsk(:,2,:) = imag(SegmentedSigBpsk);

        SegmentedSigQpsk = add3SigQpsk(1+spf*(segIdx-1):spf*segIdx,:);
    
        dataQpsk(:,1,:) = real(SegmentedSigQpsk);
        dataQpsk(:,2,:) = imag(SegmentedSigQpsk);

        SegmentedSig8psk = add3Sig8psk(1+spf*(segIdx-1):spf*segIdx,:);
    
        data8psk(:,1,:) = real(SegmentedSig8psk);
        data8psk(:,2,:) = imag(SegmentedSig8psk);

        SegmentedSig16qam = add3Sig16qam(1+spf*(segIdx-1):spf*segIdx,:);
    
        data16qam(:,1,:) = real(SegmentedSig16qam);
        data16qam(:,2,:) = imag(SegmentedSig16qam);

        SegmentedSig32qam = add3Sig32qam(1+spf*(segIdx-1):spf*segIdx,:);
    
        data32qam(:,1,:) = real(SegmentedSig32qam);
        data32qam(:,2,:) = imag(SegmentedSig32qam);

        SegmentedSig64qam = add3Sig64qam(1+spf*(segIdx-1):spf*segIdx,:);
    
        data64qam(:,1,:) = real(SegmentedSig64qam);
        data64qam(:,2,:) = imag(SegmentedSig64qam);

        SegmentedSig4pam = add3Sig4pam(1+spf*(segIdx-1):spf*segIdx,:);
    
        data4pam(:,1,:) = real(SegmentedSig4pam);
        data4pam(:,2,:) = imag(SegmentedSig4pam);

        SegmentedSig8pam = add3Sig8pam(1+spf*(segIdx-1):spf*segIdx,:);
...
%% 학습된 신경망을 통한 결과
        ...
        trainedNet      = importONNXNetwork(snr(snrIdx) + "dB_AMC_network(Add3).onnx");
