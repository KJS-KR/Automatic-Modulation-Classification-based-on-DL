...
dmrs.CustomSymbolSet        = symbolLocationType2;
...
for snrIdx = 12:length(snr)
    ...
    parfor frameIdx = 1:numFrame
        ...
        % parfor 문제로 그리드에 신호 삽입
        ResourceQpsk = vertcat(modQpskSig, ResourceGridPart);
        % DMRS 삽입
        ResourceQpsk(ind_dmrs) = 2*sym_dmrs;
        % TDL-A 채널 생성
        ...
        Resource16qam(ind_dmrs) = 2*sym_dmrs;
        H = Hout(1:N_sc,1:N_syms);
        ...
        Resource64qam(ind_dmrs) = 2*sym_dmrs;
        H = Hout(1:N_sc,1:N_syms);
        ...
        Resource256qam(ind_dmrs) = 2*sym_dmrs;
        H = Hout(1:N_sc,1:N_syms);
...
%% Training network and processing time
    ...
    exportONNXNetwork(trainedNet, "DMRS_Network_Type2/" + snr(snrIdx) + "dB_AMC_Network.onnx")
end