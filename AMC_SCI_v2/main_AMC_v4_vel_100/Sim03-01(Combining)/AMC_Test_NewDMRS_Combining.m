...
numFrame        = 50000;        % Number of total data
rep             = 2;
comb            = 1;
numSegment      = rep/comb;
...
nr_CarrierConfig.velocity       = vel; % in km/h
...
for snrIdx = 1:1 %length(snr) 
    ...
    parfor frameIdx = 1:numFrame
        ...
        softCom16qamResource = cell(numSegment, 1);
        for segIdx = 1:numSegment
            if comb == 1
                softCom16qamResource{segIdx} = awgn16qamSig{segIdx};
            elseif comb == 2
                softCom16qamResource{segIdx} = awgn16qamSig{2*(segIdx-1)+1} + awgn16qamSig{2*segIdx};
            elseif comb == 4
                softCom16qamResource{segIdx} = awgn16qamSig{4*(segIdx-1)+1} + awgn16qamSig{4*(segIdx-1)+2} + awgn16qamSig{4*(segIdx-1)+3} + awgn16qamSig{4*(segIdx-1)+4};
            end
        end
        for segIdx = 2:numSegment
            softCom16qamResource{1} = vertcat(softCom16qamResource{1}, softCom16qamResource{segIdx});
        end

        total16qamSig(:, :, frameIdx) = softCom16qamResource{1};
        ...
        softCom64qamResource = cell(numSegment, 1);
        for segIdx = 1:numSegment
            if comb == 1
                softCom64qamResource{segIdx} = awgn64qamSig{segIdx};
            elseif comb == 2
                softCom64qamResource{segIdx} = awgn64qamSig{2*(segIdx-1)+1} + awgn64qamSig{2*segIdx};
            elseif comb == 4
                softCom64qamResource{segIdx} = awgn64qamSig{4*(segIdx-1)+1} + awgn64qamSig{4*(segIdx-1)+2} + awgn64qamSig{4*(segIdx-1)+3} + awgn64qamSig{4*(segIdx-1)+4};
            end
        end
        for segIdx = 2:numSegment
            softCom64qamResource{1} = vertcat(softCom64qamResource{1}, softCom64qamResource{segIdx});
        end

        total64qamSig(:, :, frameIdx) = softCom64qamResource{1};
        ...
        softCom256qamResource = cell(numSegment, 1);
        for segIdx = 1:numSegment
            if comb == 1
                softCom256qamResource{segIdx} = awgn256qamSig{segIdx};
            elseif comb == 2
                softCom256qamResource{segIdx} = awgn256qamSig{2*(segIdx-1)+1} + awgn256qamSig{2*segIdx};
            elseif comb == 4
                softCom256qamResource{segIdx} = awgn256qamSig{4*(segIdx-1)+1} + awgn256qamSig{4*(segIdx-1)+2} + awgn256qamSig{4*(segIdx-1)+3} + awgn256qamSig{4*(segIdx-1)+4};
            end
        end
        for segIdx = 2:numSegment
            softCom256qamResource{1} = vertcat(softCom256qamResource{1}, softCom256qamResource{segIdx});
        end

        total256qamSig(:, :, frameIdx) = softCom256qamResource{1};
    end
...
%% 학습된 신경망을 통해 Fusion Method 시뮬레이션
        ...
        trainedNet      = importONNXNetwork(comb + "Comb_Network_"+ vel +"/" + snr(snrIdx) + "dB_AMC_network.onnx");
